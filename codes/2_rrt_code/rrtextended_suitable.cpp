 ...
 //c++ `Magick++-config --cxxflags --cppflags` -O2 -o rrtextended_suitable rrtextended_suitable.cpp   `Magick++-config --ldflags --libs`
#include <Magick++.h>
#include <iostream>
#include <fstream>
#include <math.h>
...
string input_type="image";   //define the input type either (image or binary)
const int m=50;    // map rows
const int n=50;    // map coloms
...
// // map information
// const char* map_image_direcotry= "/home/ganam/Desktop/final 1/250 maps/image_250_9.jpg";
// const char* map_binary_directory="/home/ganam/Desktop/final 1/250 maps/binary_250_9.bin";
// string input_type="binary";   //define the input type either (image or binary)
// const int m=250;    // map rows
// const int n=250;    // map coloms
// 
// int x_start= 15;        //  start x poistion 
// int y_start=15;        //  start y postion 
// int x_goal=230;         //  goal  x postion
// int y_goal=230;         //  goal  y postion

int threshold=100000;     // the minimum distace for node to join the tree
    ...
    if(input_type=="image")
    {
        ...
        for(int i=0;i<m;i++)
        {
            for(int j=0;j<n ;j++)
            {
                ...
                ColorRGB rgb(img.pixelColor(i, j));  // ie. pixel at pos 
                cout<<i<<","<<j<<endl;
                myarray[i][j]=rgb.red();
            }            
        }
        ...
        fstream myfile0("myfile1.bin",ios::out  | ios::trunc | ios::binary|ios::in);
        myfile0.close();
        
        // open "myfile1.bin" in suitable writing mode
        fstream myfile("myfile1.bin",ios::out  | ios::app | ios::binary |ios::in);
...
//              Image disimage("/home/ganam/Desktop/ph auto/rrt extended 50/paris.jpg");
             Image disimage(map_image_direcotry);
             disimage.pixelColor(start.y, start.x, "blue" ); 
             disimage.pixelColor(goal.y,goal.x , "red" ); 
             disimage.scale(Geometry(display_scale,display_scale, display_scale, display_scale) );
             disimage.display();
    ...
    while(/*distToGoal>=exit_conditon ||*/ pathToGoal==0)
    {
      ...
      //  cout<<"sugestedx="<<globalx<<" sugestedy="<<globaly<<endl;
        ...
        int suitable_point_flag=0;
        node minimum ;
        minimum.dist=10000000; //random larg number
        for (int i=0; i<tree_size; i=i+1)
        {                
            if(dynamic_tree[i].dist<= threshold && dynamic_tree[i].path==1)
            {                    
//              suitable_array[suitable_array_size]=dynamic_tree[i];
                if(dynamic_tree[i].dist <= minimum.dist)
                {
                    minimum=dynamic_tree[i];
                }
                suitable_point_flag=1;
            }
        } 
...
//                 }
                ...
                {  
                    ...
                    cout<<"new node added to the tree with............ ";
...
// rrt extended refines the path found getting rid of any non useful node in the path
    ...
    for(int i =1; i<ex_array_size; i=i+1)
    {

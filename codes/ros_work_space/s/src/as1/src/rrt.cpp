...
//cout<<"z ="<<modmsg ->pose[1].orientation.z<<endl;
//cout<<"th="<<th<<endl;
...
// read the nodes from the rnodes.txt file
  ifstream fin;
	//fin.open("/home/ganam/Desktop/imreadtest/astar/astar_finished/nodes.TXT");
        fin.open("/home/ganam/Desktop/imreadtest/rrt/magic++tests/rrt extended/rnodes.TXT");
...
//creating scaled x and y array from the image to truth scale
        ...
        {
           xDestarray[i]=xarray[i]/scale;
           yDestarray[i]=yarray[i]/scale;
...
{  
    ...
    { 
        ...
        thdiff=thg-th;
        if(th<90 &&  thg<360 && thdiff>270) {thdiff=thg-360-th; cout<<11111111<<endl;} // those two lines to  get over the transitioin between 
        if(thg<90 && th<360 && th>270     ) {thdiff=thg+360-th; cout<<22222222<<endl;}//  angels less than 360 and more than 0 
...
//cout<<"x ="<<x<< "   "<<"y ="<<y<<"   " <<"th="<<th<<"   "<<"thg="<<thg<<endl;
  ...
  cout<<" dist = "<< dist<<endl;      
        ...
        if ( abs(thdiff)>7) // checking the angel is still near the right direction 
          {
           geom.linear.x=0;  geom.linear.y=0; geom.linear.z=0;  geom.angular.x=0; geom.angular.y=0; 
           geom.angular.z=angsped*0;
           publisher.publish(geom);
